...
router.get("/post/:slug", async (req, res) => {
  try {
    ...
    let recents = await Post.find({ slug: { $nin: post.slug } })
...
router.put("/post/:slug", async (req, res) => {
  try {
    ...
    if (!post) {
      req.flash(
        "error_msg",
        "This post might have been deleted or other errors"
      );
...
router.get("/search", async (req, res) => {
  try {
    ...
    let search = await Post.find({ $or: [{ title: q }, { detail: q }] })
      .sort({ date: -1 })
      .skip(limit * page - limit)
      .limit(limit);
